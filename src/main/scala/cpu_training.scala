...
object CPU_Training{
  def euclid_dist_2(numCoords: Int, numObjs: Int, numClusters: Int, objects: Array[Float], clusters: Array[Float], objectId: Int, clusterId: Int): Float = {
    var ans = 0.0f
  ...
  def find_nearest_cluster(numCoords: Int, numObjs: Int, numClusters: Int, objects: Array[Float], deviceClusters: Array[Float], membership: Array[Int], intermediates: Array[Int]) = {
    var membershipChanged = Array.fill(numObjs){0}
    var clusters = deviceClusters

    for (objectId <- 0 until numObjs){
      
      var index = 0
      var min_dist = euclid_dist_2(numCoords, numObjs, numClusters, objects, clusters, objectId, 0 )

      for (i <- 1 until numClusters){
        val dist = euclid_dist_2(numCoords, numObjs, numClusters, objects, clusters, objectId, i)
        if (dist < min_dist) {
          min_dist = dist
          index = i
        }
    ...
    for(i <- 0 until numObjs){
      intermediates(0) += membershipChanged(i)
    }
  }
...
//        cuLaunchKernel(function1, numClusterBlocks, 1, 1, numThreadsPerClusterBlock, 1, 1, clusterBlockSharedDataSize, null, kernelParameters1, null)
          ...
          val d = Array(0)
          find_nearest_cluster(numCoords, numObjs, numClusters, dimObjects, dimClusters, membership, d)
//
